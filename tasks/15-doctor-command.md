...
- Warnings must explain **exactly how to fix them** (e.g. install command or URL). Reference the documented install commands (see `docs/requirements.md`) — on Windows, RTK/LeanCTX install via GitHub release zips (the Linux `install.sh` / `npm i -g lean-ctx-bin` routes do not work on Windows).
