...
- Package name: `cadet-token-saver` (per the `npx cadet-token-saver ...` CLI examples in the design doc).
...
- Add `.gitignore` (node_modules, dist, `.cadet-token-saver/` local state, SQLite db files).
...
- [ ] Package installs/resolves via `npx cadet-token-saver` without error.
