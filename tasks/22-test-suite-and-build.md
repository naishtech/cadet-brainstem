...
**Source:** `docs/plans/initial_design.md` — §17 Development approach ("After each phase, write tests", "Prefer integration tests using mocked external tools")
...
- Final result must be a real npm package usable via `npx token-optimizer`, not a mock-only prototype.
...
- [ ] `npx token-optimizer init` works from the packaged artifact.
